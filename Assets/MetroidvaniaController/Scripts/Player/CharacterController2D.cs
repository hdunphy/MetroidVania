...
public class CharacterController2D : MonoBehaviour
{
    ...
    const float k_WallRadius = .2f; // Radius of the overlap circle to determine if touching a wall
    ...
    private bool hasDash = true; //to prevent dashing in mid air unlimited times
    ...
    private void FixedUpdate()
    {
        ...
        if (!m_Grounded)
        {
            ...
            prevVelocityX = m_Rigidbody2D.velocity.x; //Not being used (henry)
        }
    ...
    public void Move(float move, bool jump, bool dash)
    {
        if (canMove)
        {
            if (dash && canDash && hasDash && !isWallSliding)
            {
